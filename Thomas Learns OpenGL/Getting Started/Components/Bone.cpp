...
Bone::Bone(const std::string& name, int ID, const aiNodeAnim* channel)
	...
	for (int positionIndex = 0; positionIndex < m_NumPositions; positionIndex++) {
		...
		// create data for positions vector
		KeyPosition data;
	...
	// repeat for bone rotations
	m_NumRotations = channel->mNumRotationKeys;
	for (int rotationIndex = 0; rotationIndex < m_NumRotations; rotationIndex++) {
		aiQuaternion aiOrientation = channel->mRotationKeys[rotationIndex].mValue;
		float timeStamp = channel->mRotationKeys[rotationIndex].mTime;
		KeyRotation data;
		data.orientation = AssimpGLMHelpers::GetQuat(aiOrientation);
		data.timeStamp = timeStamp;
		m_Rotations.push_back(data);
	}

	// repeat for bone scalings
...
glm::mat4 Bone::InterpolateRotation(float animationTime) {
	// only one rotation, use it
	if (1 == m_NumRotations) {
		auto rotation = normalize(m_Rotations[0].orientation);
		return QuatToMat4(rotation);
	}
	// interpolation between p0 and p1
	int p0Index = GetRotationIndex(animationTime);
	int p1Index = p0Index + 1;
	float scaleFactor = GetScaleFactor(m_Rotations[p0Index].timeStamp, m_Rotations[p1Index].timeStamp, animationTime);
	// use Slerp function from Quaternion class
	...
	return QuatToMat4(finalRotation);
}

glm::mat4 Bone::InterpolateScaling(float animationTime) {
	// only one scaling, use it
	if (1 == m_NumScalings) {
